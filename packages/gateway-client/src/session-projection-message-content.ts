...
export function readSessionMessageDisplayContent(message: unknown): {
  ...
  for (const block of Array.isArray(content) ? content : [content]) {
    ...
    if (entry && entry.type !== "text" && entry.type !== "output_text") {
      hasNonText = true;
      continue;
    }
  ...
  const fallback = readNonemptyString(record?.text);
  const usesFallbackText = texts.length === 0 && fallback !== null;
  if (usesFallbackText) {
    texts.push(fallback);
  }
  return { text: texts.join("\n"), hasNonText, usesFallbackText };
}
