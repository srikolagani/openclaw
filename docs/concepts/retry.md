...
Embedded model runs use the [model failover controller](/concepts/model-failover#model-fallback) for transient HTTP-response retries. Rate limits receive up to 10 total attempts, with jittered exponential backoff capped at 30 seconds and provider `retry-after` / `retry-after-ms` hints as the minimum wait. Other transient errors retain their configured retry budget and 90-second retry window. The Control UI shows one transient retry indicator; recovered attempts do not leave error messages in history. ChatGPT SSE errors preserve HTTP status and `Retry-After` together, so a transient HTTP response remains retryable even when its message or provider code is unfamiliar. The ChatGPT transport separately reconnects once for `websocket_connection_limit_reached` before streaming; this is not an SSE HTTP-response retry.
