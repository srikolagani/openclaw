...
export function createSystemAgentTool(options: SystemAgentToolOptions): AnyAgentTool {
  return {
    ...
    description: [
      ...
      "plugin_activate_artifact: for a task-authored plugin built with openclaw plugins pack, pass its absolute archive path and sha256. Copies and reviews exact bytes before proposing; approval includes trusted backend code, declared capabilities, and native UI. No dependency fetching. Applies backend changes through the running Gateway's plugin lifecycle. Native UI separately requires enabling Settings > Labs > Custom plugin UI, then Gateway restart and browser reload; artifact approval does not enable Labs.",
